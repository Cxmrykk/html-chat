...
Consider the following website as inspiration:
...
I want to create a single-file HTML site which "just works" for chatting with OpenAI API compatible LLMs. The features will be the following:
- Save an API key and base URL pair (which stores itself in cache) along with multiple LLM IDs for that pair (e.g. `gpt-4` or something)
- Automatically save new chats and allow users to delete and rename them
- Format the user's prompts and the LLM's responses automatically (they are considered to be markdown, so convert markdown to valid HTML)
- Allow editing of prompts, responses, and also removal of certain messages in the chat.
- A "fork" button which starts a new chat with the messages up to that point.